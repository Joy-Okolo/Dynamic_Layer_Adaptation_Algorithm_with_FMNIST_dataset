...
# --- 1. Fashion-MNIST Optimized Neural Network ---
class FashionMNISTNet(nn.Module):
    def __init__(self, input_size=784, hidden_sizes=[512, 256, 128, 64], num_classes=10):
        ...
        for hidden_size in hidden_sizes:
            ...
            layers.append(nn.Dropout(0.2))  # Light dropout for Fashion-MNIST
...
def adjust_layers_improved(elapsed_time, current_depth, client_id, 
                         T_low=4.0, T_high=12.0, max_layers=5):
    """
    Improved layer adjustment with lenient thresholds for Fashion-MNIST
    """
    # More lenient, client-specific thresholds for Fashion-MNIST
...
# --- 7. Create federated Fashion-MNIST dataset ---
def create_federated_fashion_mnist(num_clients=5, batch_size=32, iid=True):
    """
    Create federated Fashion-MNIST dataset with real data
    """
    print("Downloading Fashion-MNIST dataset...")
    
    # Fashion-MNIST preprocessing (similar to MNIST)
    ...
    print("Creating federated Fashion-MNIST data splits...")
    
    # Fashion-MNIST class names for non-IID splits
    ...
    for client_id in range(num_clients):
        ...
        if client_id == 0:  # Slow client with more data
            ...
            speed_factor = np.random.uniform(0.5, 1.8)
        ...
        if iid:
            ...
            print(f"Client {client_id} fashion items: {[fashion_classes[i] for i in target_classes]}")
...
def federated_training_loop(
    ...
    """
    Federated training with heterogeneous clients for Fashion-MNIST
    """
    ...
    # Initialize client depths for DLA - start at depth 3 consistently
    current_depths = {cid: 3 for cid in range(len(clients))}  # Start all at depth 3
    
    # Initialize DLA stability tracker
    dla_tracker = DLAStabilityTracker(len(clients)) if strategy == "dla" else None
    
    print(f"\nStarting Federated Training with {strategy.upper()} strategy on Fashion-MNIST")
        ...
        for cid, (train_loader, dataset_size) in enumerate(clients):
            ...
            if strategy == "dla":
                suggested_depth = adjust_layers_improved(elapsed_time, layer_depth, cid,
                                                       T_low=4.0, T_high=12.0, max_layers=max_layers)
...
def plot_comparison_results_improved(all_results, strategies):
    """
    Plot comparison with improved visibility and styling for Fashion-MNIST
    """
    ...
    axes[0, 0].set_title('Test Accuracy Over Rounds (Fashion-MNIST)', fontsize=14, fontweight='bold')
...
def plot_accuracy_only(all_results, strategies):
    """
    Dedicated accuracy plot with maximum visibility for Fashion-MNIST
    """
    ...
    plt.title('Fashion-MNIST Test Accuracy: Federated Learning Strategies', fontsize=16, fontweight='bold', pad=20)
...
def run_fashion_mnist_federated_experiment(iid=True, num_rounds=18):
    """
    Run comprehensive federated learning experiment with Fashion-MNIST
    """
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    
    # Create federated Fashion-MNIST data
    ...
    print(f"\n{'='*70}")
    print(f"FEDERATED LEARNING EXPERIMENT ON FASHION-MNIST ({data_type})")
    print(f"{'='*70}")
    ...
    for strategy in strategies:
        ...
        global_model, results = federated_training_loop(
            ...
            device=device,
            strategy=strategy,
            max_layers=5,  # Fashion-MNIST has 5 linear layers
            fixed_depth=3,
            test_loader=test_loader,
            lr=0.001,  # Learning rate optimized for Fashion-MNIST
            local_epochs=2  # Good balance for fashion items
        )
...
if __name__ == "__main__":
    ...
    print("Running Fashion-MNIST Federated Learning Experiment...")
    results_iid = run_fashion_mnist_federated_experiment(iid=True, num_rounds=18)
    
    # Uncomment to also run with Non-IID data
    # print("\n" + "="*70)
    # print("Running Non-IID experiment...")
    # results_noniid = run_fashion_mnist_federated_experiment(iid=False, num_rounds=18)
